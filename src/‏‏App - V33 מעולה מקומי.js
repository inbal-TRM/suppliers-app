/* global window, indexedDB, FileReader, Image, fetch, document, URL, Blob, Array, parseInt, isNaN, setTimeout */
...
function getProvince(city){
  if(!city) return "";
  const k = CITIES.find(c=>c.toLowerCase()===city.trim().toLowerCase());
  return k ? CPM[k] : "";
}
...
function CityInput({value,onChange,onCitySelect}){
  ...
  function handle(e){
    ...
    if(matched&&matched!==prevVal.current){prevVal.current=matched;onCitySelect(matched);}
  }
  return(
    <div>
      <input value={inputVal} list={listId.current} onChange={handle} onInput={handle}
        placeholder="הקלד שם עיר..." autoComplete="off"
...
function MultiSelect({options,value=[],onChange}){
  ...
  const toggle=opt=>{
    if(opt==="אחר"){if(hasOther)onChange(value.filter(v=>coreOpts.includes(v)));else onChange([...value.filter(v=>coreOpts.includes(v)),"אחר"]);}
    else{if(value.includes(opt))onChange(value.filter(v=>v!==opt));else onChange([...value.filter(v=>v!=="אחר"&&!customVals.includes(v)),opt,...(hasOther?[otherText||"אחר"].filter(Boolean):[])]);}
  };
  return(
...
function ProvinceSelect({value,onChange}){
  ...
  return(
    <div>
      <select value={selVal} onChange={e=>onChange(e.target.value)}
...
function useDB(){
  ...
  useEffect(()=>{
    const req=indexedDB.open("suppliers_db_v11",1);
  ...
  return{
    ready,
    getAllSuppliers:()=>tx("suppliers","readonly",s=>s.getAll()),
    addSupplier:d=>tx("suppliers","readwrite",s=>s.add(d)),
    updateSupplier:d=>tx("suppliers","readwrite",s=>s.put(d)),
    deleteSupplier:id=>tx("suppliers","readwrite",s=>s.delete(id)),
    getProductsBySupplier:sid=>new Promise((res,rej)=>{const t=dbRef.current.transaction("products","readonly"),s=t.objectStore("products"),r=s.index("supplierId").getAll(sid);r.onsuccess=()=>res(r.result);r.onerror=()=>rej(r.error);}),
    addProduct:d=>tx("products","readwrite",s=>s.add(d)),
    updateProduct:d=>tx("products","readwrite",s=>s.put(d)),
    deleteProduct:id=>tx("products","readwrite",s=>s.delete(id)),
    getAllProducts:()=>tx("products","readonly",s=>s.getAll()),
...
function LoginScreen({onLogin}){
  ...
  async function checkModels(){
    if(!key.trim())return;
    setTesting(true);setKeyErr("");setModels([]);
    try{
      const res=await fetch(`https://generativelanguage.googleapis.com/v1beta/models?key=${key.trim()}`);
      const d=await res.json();
      if(d.error){setKeyErr(d.error.message);setTesting(false);return;}
      const list=(d.models||[]).filter(m=>(m.supportedGenerationMethods||[]).includes("generateContent")).map(m=>m.name.replace("models/",""));
      setModels(list);
      if(list.length===0)setKeyErr("לא נמצאו מודלים זמינים");
    ...
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#f1f5f9",fontFamily:"sans-serif"}}>
      <div style={{background:"#fff",borderRadius:18,border:"1px solid rgba(0,0,0,0.1)",padding:"2rem",width:320,direction:"rtl"}}>
        ...
        <div style={{position:"relative",marginBottom:8}}>
          <input value={key} onChange={e=>{setKey(e.target.value);setKeyErr("");setModels([]);}}
            type={showKey?"text":"password"} placeholder="AIza..."
            style={{width:"100%",boxSizing:"border-box",padding:"9px 12px",paddingLeft:36,fontSize:13,borderRadius:9,border:`1.5px solid ${keyErr?"#dc2626":"rgba(0,0,0,0.18)"}`,outline:"none",textAlign:"left",direction:"ltr"}}/>
        ...
        <button onClick={checkModels} disabled={!key.trim()||testing}
          style={{width:"100%",padding:"8px",borderRadius:9,border:"1.5px solid #2563eb",background:"#eff6ff",cursor:"pointer",fontSize:13,fontWeight:500,color:"#1d4ed8",marginBottom:8}}>
          {testing?"בודק...":"🔍 בדוק מודלים זמינים"}
        </button>
        {models.length>0&&(
          <div style={{background:"#f0fdf4",border:"1px solid #bbf7d0",borderRadius:8,padding:"8px 10px",marginBottom:8,fontSize:12,color:"#065f46"}}>
            <div style={{fontWeight:600,marginBottom:4}}>מודלים זמינים ({models.length}):</div>
            {models.map(m=><div key={m} style={{fontFamily:"monospace",fontSize:11}}>{m}</div>)}
          </div>
        )}
...
export default function App(){
  ...
  const loadAll=useCallback(async()=>{
    ...
    if(userMeta?.value&&!currentUser)setCurrentUser(userMeta.value);
  },[db.ready]);
  ...
  const handleCardFile=async file=>{
    ...
    try{
      ...
      if(!currentUser?.key){showMsg("אין Gemini API key",false);setExtracting(false);return;}
      ...
      try{
        ...
        setForm(f=>({...f,name:p.name||f.name,contact:p.contact||f.contact,phone:p.phone||f.phone,email:p.email||f.email,city:matchedCity||f.city,province:autoProvince||f.province}));
        showMsg("פרטים חולצו בהצלחה ✓",true);
      }catch(_){showMsg("לא הצלחתי לפרסר את התשובה",false);}
  ...
  const handleDelSup=async id=>{
    ...
    await db.deleteSupplier(id);await loadAll();
  ...
  const handleDelProd=async id=>{
    ...
    await db.deleteProduct(id);await loadProducts(selSup.id);
  };
  ...
  const buildCSV=async()=>{
    ...
    allS.forEach(s=>{csv+=[s.id,`"${(s.name||"").replace(/"/g,'""')}"`,`"${(s.source||"").replace(/"/g,'""')}"`,s.date||"",`"${(s.contact||"").replace(/"/g,'""')}"`,s.phone||"",s.email||"",s.city||"",s.province||"",`"${(s.fields||[]).join(", ")}"`,`"${(s.description||"").replace(/"/g,'""')}"`,s.rating||0,s.createdAt||"",s.createdBy||"",s.updatedAt||"",s.updatedBy||""].join(",")+"\n";});
  ...
  const downloadZip=async()=>{
    ...
    try{
      ...
      zip.file(`${folder}/suppliers_${folder}.csv`,await buildCSV());
      const allS=await db.getAllSuppliers(),allP=await db.getAllProducts();
      for(const s of allS){if(s.cardImage){const c=await compressImage(s.cardImage,s.cardImageType||"image/jpeg");zip.file(`${folder}/${s.id}.card.jpg`,c.data,{base64:true});}}
      for(const p of allP){for(let i=0;i<(p.images||[]).length;i++){const img=p.images[i];const c=await compressImage(img.data,img.type||"image/jpeg");zip.file(`${folder}/${p.supplierId}.${p.id}.${i+1}.jpg`,c.data,{base64:true});}}
      const blob=await zip.generateAsync({type:"blob",compression:"DEFLATE",compressionOptions:{level:6}});
      const a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download=`suppliers_${folder}.zip`;a.click();
      showMsg(`✓ הורד: suppliers_${folder}.zip`,true);
    }catch(e){showMsg("שגיאה: "+e.message,false);}
  };
  ...
  const importSuppliers=async file=>{
    ...
    try{
      ...
      for(let i=start;i<lines.length;i++){
        ...
        await db.addSupplier({name:c[1]||"",source:c[2]||"",date:c[3]||todayStr(),contact:c[4]||"",phone:c[5]||"",email:c[6]||"",city:c[7]||"",province:c[8]||"",fields:(c[9]||"").split(",").map(s=>s.trim()).filter(Boolean),description:c[10]||"",rating:parseInt(c[11])||0,cardImage:"",cardImageType:"",createdAt:now,createdBy:"import",updatedAt:now,updatedBy:"import"});
        count++;
      }
  ...
  const importImages=async files=>{
    ...
    for(const file of Array.from(files)){
      const name=file.name.replace(/\.[^.]+$/,"");const parts=name.split(".");
    ...
    <div dir="rtl" style={{fontFamily:"var(--font-sans)",padding:"0.75rem",maxWidth:560,margin:"0 auto",background:"var(--color-background-tertiary)",minHeight:"100vh"}}>
      ...
      {view==="list"&&<>
        <Section title="ייצוא נתונים" icon="📤" accent="#0f766e">
          <div style={{display:"flex",gap:10}}><ActBtn icon="📦" label="הורד ZIP" bg="#f0fdf4" color="#065f46" onClick={downloadZip}/></div>
          <div style={{fontSize:11,color:"#888",marginTop:8}}>ZIP עם תיקיית תאריך+שעה: CSV + תמונות מכווצות</div>
        </Section>
        <Section title="ייבוא נתונים" icon="📥" accent="#7c3aed">
          ...
          <div style={{fontSize:11,color:"#888",marginTop:8}}>תמונות: ספקID.card.jpg | ספקID.מוצרID.מספר.jpg</div>
        </Section>
      ...
      {view==="form"&&<>
        <div style={cardStyle}>
          ...
          <div style={{marginBottom:18}}>
            ...
            <div style={{display:"flex",gap:12,alignItems:"center",flexWrap:"wrap",padding:"12px",background:"#f8f7f5",borderRadius:10,border:"1.5px dashed rgba(0,0,0,0.15)"}}>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:6}}>
                ...
                {extracting&&<span style={{fontSize:12,color:"#2563eb",fontWeight:500}}>Gemini מחלץ פרטים...</span>}
              </div>
            </div>
          </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            <Field label="טלפון"><input style={IS} value={form.phone||""} onChange={e=>sf("phone",e.target.value)} placeholder="+86..." type="tel"/></Field>
            <Field label="אימייל"><input style={IS} value={form.email} onChange={e=>sf("email",e.target.value)} type="email" placeholder="email@..."/></Field>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            <Field label="עיר"><CityInput value={form.city} onChange={v=>sf("city",v)} onCitySelect={city=>{sf("city",city);const p=getProvince(city);if(p)sf("province",p);}}/></Field>
      ...
      {editImgCtx?.target==="prod"&&(()=>{
        const idx=editImgCtx.idx,img=prodForm.images?.[idx];if(!img)return null;
