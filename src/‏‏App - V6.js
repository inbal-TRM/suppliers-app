...
/* ── City → Province map (exact, trimmed) ── */
...
/* Compress image to JPEG at given quality, returns {data:base64, type:"image/jpeg"} */
function compressImage(b64,srcType,quality=0.72){
  return new Promise(res=>{
    ...
    img.onload=()=>{
      ...
      const url=cvs.toDataURL("image/jpeg",quality);
      res({data:url.split(",")[1],type:"image/jpeg"});
    };
    img.src=`data:${srcType};base64,${b64}`;
  });
}
...
function CityInput({value,onChange,onCitySelect}){
  ...
  const matches=q.length>0?CITIES.filter(c=>c.toLowerCase().startsWith(q.toLowerCase())).concat(CITIES.filter(c=>!c.toLowerCase().startsWith(q.toLowerCase())&&c.toLowerCase().includes(q.toLowerCase()))):[];
  return(
    <div style={{position:"relative"}}>
      <input value={q}
        onChange={e=>{const v=e.target.value;setQ(v);onChange(v);setShow(true);}}
        onFocus={()=>q.length>0&&setShow(true)}
      ...
      {show&&matches.length>0&&(
        <div style={{position:"absolute",top:"100%",right:0,left:0,background:"#fff",border:"1.5px solid rgba(0,0,0,0.14)",borderRadius:8,zIndex:200,maxHeight:180,overflowY:"auto",boxShadow:"0 4px 16px rgba(0,0,0,0.12)"}}>
          {matches.map(c=>(
            <div key={c} onMouseDown={()=>{setQ(c);setShow(false);onCitySelect(c);}}
              style={{padding:"9px 12px",cursor:"pointer",fontSize:14,borderBottom:"0.5px solid rgba(0,0,0,0.06)",display:"flex",justifyContent:"space-between",alignItems:"center"}}
              onMouseEnter={e=>e.currentTarget.style.background="#f0f7ff"}
              onMouseLeave={e=>e.currentTarget.style.background=""}>
              <span>{c}</span>
              <span style={{fontSize:12,color:"#2563eb",fontWeight:500}}>{CITY_PROVINCE_MAP[c]}</span>
            </div>
          ))}
        </div>
      )}
...
function MultiSelect({options,value=[],onChange}){
  ...
  const hasOther=value.some(v=>!coreOpts.includes(v));
  const otherVal=value.find(v=>!options.includes(v))||"";
  const toggle=opt=>{
    if(opt==="אחר"){
      if(hasOther)onChange(value.filter(v=>options.includes(v)&&v!=="אחר"));
      else onChange([...value.filter(v=>coreOpts.includes(v)),"אחר"]);
    } else {
      if(value.includes(opt))onChange(value.filter(v=>v!==opt));
      else onChange([...value.filter(v=>v!=="אחר"&&!(!options.includes(v))),opt,...(hasOther?["אחר",otherVal].filter(Boolean):[])]);
    }
  };
  return(
    <div style={{padding:"10px"}}>
      ...
      {hasOther&&<input value={otherVal} onChange={e=>onChange([...value.filter(v=>coreOpts.includes(v)&&v!=="אחר"),e.target.value])} placeholder="הקלד תחום..." style={{width:"100%",boxSizing:"border-box",padding:"7px 10px",fontSize:13,borderRadius:8,border:"1.5px solid rgba(0,0,0,0.18)",outline:"none",background:"#f8f8f8"}}/>}
    </div>
  );
}
...
function ProvinceSelect({value,onChange}){
  ...
  return(
    <div>
      <select value={selVal} onChange={e=>onChange(e.target.value)} style={{width:"100%",boxSizing:"border-box",padding:"9px 11px",fontSize:14,border:"none",outline:"none",background:"transparent",color:"#111",fontFamily:"inherit"}}>
        <option value="">-- בחר מחוז --</option>
        {PROVINCES.map(p=><option key={p} value={p}>{p}</option>)}
      </select>
      {(selVal==="אחר"||isCustom)&&<input value={isCustom?value:""} onChange={e=>onChange(e.target.value)} placeholder="הקלד מחוז..." style={{width:"100%",boxSizing:"border-box",padding:"8px 11px",fontSize:13,border:"none",borderTop:"1px solid rgba(0,0,0,0.1)",outline:"none",background:"#f8f8f8",color:"#111"}}/>}
    </div>
  );
}
...
function useDB(){
  ...
  useEffect(()=>{
    const req=indexedDB.open("suppliers_db_v8",1);
...
/* ── Section card ── */
function Section({title,icon,children,accent="#1d4ed8"}){
  return(
...
/* ── Action Button ── */
function ActBtn({icon,label,onClick,color="#333",bg="#fff"}){
  return(
    <button onClick={onClick} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:4,padding:"10px 14px",borderRadius:10,border:"1.5px solid rgba(0,0,0,0.14)",background:bg,cursor:"pointer",color,fontWeight:500,fontSize:12,minWidth:70}}>
...
export default function App(){
  ...
  const [msgColor,setMsgColor]=useState("#1d4ed8");
  const importSupRef=useRef();
  const importImgRef=useRef();

  const showMsg=(text,color="#1d4ed8")=>{setMsg(text);setMsgColor(color);setTimeout(()=>setMsg(""),4000);};
  ...
  const handleCardFile=async file=>{
    ...
    try{
      ...
      const txt=await callClaude("Business card image. Extract: name (company), contact (person name), email, city (English city name only). Return ONLY valid JSON with keys: name, contact, email, city. No markdown.",b64,mt);
      try{
        const p=JSON.parse(txt.replace(/```json|```/g,"").trim());
        const rawCity=(p.city||"").trim();
        // Find matching city (case-insensitive, trimmed)
        const matchedCity=CITIES.find(c=>c.toLowerCase()===rawCity.toLowerCase())||rawCity;
        const autoProvince=CITY_PROVINCE_MAP[matchedCity]||CITY_PROVINCE_MAP[CITIES.find(c=>c.toLowerCase()===rawCity.toLowerCase())]||"";
        setForm(f=>({...f,
          name:p.name||f.name,contact:p.contact||f.contact,email:p.email||f.email,
  ...
  const handleSave=async()=>{
    ...
    try{
      ...
      if(selSup){
        await db.updateSupplier({...form,id:selSup.id,updatedAt:now,updatedBy:"user"});
      } else {
        const id=await db.addSupplier({...form,createdAt:now,createdBy:"user",updatedAt:now,updatedBy:"user"});
      ...
      showMsg("הספק נשמר בהצלחה","#059669");
    }catch(e){console.error(e);showMsg("שגיאה בשמירה","#dc2626");}
    setSaving(false);
  };
...
/* eslint-disable no-restricted-globals */
  ...
  const handleSaveProd=async()=>{
    ...
    const now=nowISO();
    const data={...prodForm,supplierId:sid,updatedAt:now,updatedBy:"user"};
    if(editProd)await db.updateProduct({...data,id:editProd.id});
    else await db.addProduct({...data,createdAt:now,createdBy:"user"});
  ...
  const exportCSV=async()=>{
    ...
    let csv="\uFEFFאינדקס,שם ספק,מקור,תאריך,איש קשר,אימייל,עיר,מחוז,תחום עיסוק,תיאור,דירוג,נוצר,עודכן\n";
    allS.forEach(s=>{csv+=[s.id,`"${(s.name||"").replace(/"/g,'""')}"`,`"${(s.source||"").replace(/"/g,'""')}"`,s.date||"",`"${(s.contact||"").replace(/"/g,'""')}"`,s.email||"",s.city||"",s.province||"",`"${(s.fields||[]).join(", ")}"`,`"${(s.description||"").replace(/"/g,'""')}"`,s.rating||0,s.createdAt||"",s.updatedAt||""].join(",")+"\n";});
    csv+="\n\nמוצרים\nאינדקס מוצר,ספק ID,תיאור,דירוג,נוצר,עודכן\n";
    allP.forEach(p=>{csv+=[p.id,p.supplierId,`"${(p.description||"").replace(/"/g,'""')}"`,p.rating||0,p.createdAt||"",p.updatedAt||""].join(",")+"\n";});
    return csv;
  };

  /* ── Download all (CSV + images) in "folder" ── */
  const downloadAll=async()=>{
    showMsg("מכין קבצים להורדה...");
    const label=dtLabel();
    ...
    csvA.download=`${label}/suppliers.csv`;
    csvA.click();
    ...
    for(const s of allS){
      if(s.cardImage){
        const compressed=await compressImage(s.cardImage,s.cardImageType||"image/jpeg");
        links.push({name:`${label}/${s.id}.card.jpg`,data:compressed.data,type:"image/jpeg"});
      }
    }
    for(const p of allP){
      for(let i=0;i<(p.images||[]).length;i++){
        const img=p.images[i];
        const compressed=await compressImage(img.data,img.type||"image/jpeg");
        links.push({name:`${label}/${p.supplierId}.${p.id}.${i+1}.jpg`,data:compressed.data,type:"image/jpeg"});
      }
    }
    ...
    showMsg(`הורדו ${links.length} תמונות + קובץ CSV בתיקייה: ${label}`,"#059669");
  };
  ...
  const importSuppliers=async file=>{
    ...
    try{
      ...
      await loadAll();showMsg(`יובאו ${count} ספקים בהצלחה`,"#059669");
    }catch(e){showMsg("שגיאה בייבוא: "+e.message,"#dc2626");}
  };

  /* ── Import images ── */
  const importImages=async files=>{
    showMsg("מייבא תמונות...");
    let count=0;
    for(const file of Array.from(files)){
      ...
      if(parts.length===2&&parts[1]==="card"){
        ...
        if(!isNaN(sid)){const all=await db.getAllSuppliers();const sup=all.find(s=>s.id===sid);if(sup){const b64=await fileToBase64(file);await db.updateSupplier({...sup,cardImage:b64,cardImageType:file.type,updatedAt:nowISO(),updatedBy:"import"});count++;}}
      } else if(parts.length>=2){
        const sid=parseInt(parts[0]),pid=parseInt(parts[1]);
        if(!isNaN(sid)&&!isNaN(pid)){
          const allP=await db.getAllProducts();const prod=allP.find(p=>p.id===pid&&p.supplierId===sid);
          if(prod){const b64=await fileToBase64(file);const imgs=[...(prod.images||[]),{data:b64,type:file.type}];await db.updateProduct({...prod,images:imgs,updatedAt:nowISO(),updatedBy:"import"});count++;}
        }
      }
    }
    await loadAll();showMsg(`יובאו ${count} תמונות`,"#059669");
  };
  ...
  return(
    <div dir="rtl" style={{fontFamily:"var(--font-sans)",padding:"0.75rem",maxWidth:560,margin:"0 auto",background:"var(--color-background-tertiary)",minHeight:"100vh"}}>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12,paddingTop:4}}>
        ...
        {view==="list"&&(
          <button onClick={()=>{setForm({...EMPTY_FORM,source:defaultSource,date:todayStr()});setSelSup(null);setErrors({});setView("form");}} style={{...btnPrimary,padding:"7px 16px",fontSize:13}}>+ ספק חדש</button>
        )}
      </div>

      {/* Msg */}
      {msg&&<div style={{background:"#f0fdf4",border:"1px solid #bbf7d0",borderRadius:8,padding:"8px 14px",marginBottom:10,fontSize:13,color:msgColor,fontWeight:500}}>{msg}</div>}
      ...
      {view==="list"&&<>
        ...
        <Section title="ייצוא נתונים" icon="📤" accent="#0f766e">
          <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
            <ActBtn icon="📊" label="ייצוא CSV" bg="#f0fdf4" color="#065f46" onClick={async()=>{const csv=await exportCSV();const a=document.createElement("a");a.href=URL.createObjectURL(new Blob([csv],{type:"text/csv;charset=utf-8;"}));a.download=`suppliers_${dtLabel()}.csv`;a.click();}}/>
            <ActBtn icon="🖼" label="הורד תמונות" bg="#f0fdf4" color="#065f46" onClick={downloadAll}/>
          </div>
          <div style={{fontSize:11,color:"#888",marginTop:8}}>תמונות יורדו דחוסות. שם קובץ: ספקID.מוצרID.מספר.jpg</div>
        </Section>
        ...
        <Section title="ייבוא נתונים" icon="📥" accent="#7c3aed">
          ...
          <div style={{fontSize:11,color:"#888",marginTop:8}}>ייבוא תמונות לפי שם: ספקID.card.jpg או ספקID.מוצרID.מספר.jpg</div>
        </Section>
        ...
        <div style={{fontSize:12,color:"#888",marginBottom:8,paddingRight:2}}>{filtered.length} ספקים</div>
        ...
        {filtered.map(s=>(
          <div key={s.id} style={{...cardStyle,display:"flex",alignItems:"center",gap:12,padding:"12px 14px"}}>
            ...
            <div style={{flex:1,minWidth:0}}>
              ...
              {(s.fields||[]).length>0&&<div style={{display:"flex",flexWrap:"wrap",gap:3,marginBottom:2}}>{s.fields.filter(f=>f).map((f,i)=><span key={i} style={{fontSize:11,padding:"2px 7px",borderRadius:20,background:"#eff6ff",color:"#1d4ed8",fontWeight:500}}>{f}</span>)}</div>}
              <Stars value={s.rating}/>
              {s.updatedAt&&<div style={{fontSize:10,color:"#bbb",marginTop:2}}>עודכן: {new Date(s.updatedAt).toLocaleString("he-IL")}</div>}
            </div>
      ...
      {view==="form"&&<>
        <div style={cardStyle}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            ...
            <Field label="עיר">
              <CityInput value={form.city} onChange={v=>sf("city",v)}
                onCitySelect={c=>{
                  sf("city",c);
                  const p=CITY_PROVINCE_MAP[c];
                  if(p){sf("province",p);}
                }}/>
            </Field>
          </div>
          ...
          {selSup&&<div style={{fontSize:11,color:"#bbb",marginBottom:12}}>
            {selSup.createdAt&&<span>נוצר: {new Date(selSup.createdAt).toLocaleString("he-IL")}  </span>}
            {selSup.updatedAt&&<span>עודכן: {new Date(selSup.updatedAt).toLocaleString("he-IL")}</span>}
          </div>}
                ...
                return(
                  <div key={p.id} style={{border:"1px solid rgba(0,0,0,0.1)",borderRadius:12,overflow:"hidden",background:"#fafaf9"}}>
                    ...
                    <div style={{padding:"10px"}}>
                      ...
                      {p.updatedAt&&<div style={{fontSize:10,color:"#ccc",marginBottom:6}}>עודכן: {new Date(p.updatedAt).toLocaleString("he-IL")}</div>}
