...
function CityInput({value, onChange, onCitySelect}){
  ...
  const suggestions = inputVal.trim().length > 0
    ? CITIES.filter(c => c.toLowerCase().includes(inputVal.trim().toLowerCase()))
    : [];
  ...
  return(
    <div ref={wrapRef} style={{position:"relative"}}>
      ...
      {open && suggestions.length > 0 && (
        <div style={{position:"absolute",top:"100%",right:0,left:0,background:"#fff",border:"1.5px solid rgba(0,0,0,0.15)",borderRadius:8,zIndex:300,maxHeight:190,overflowY:"auto",boxShadow:"0 6px 20px rgba(0,0,0,0.13)"}}>
          {suggestions.map(c=>(
            <div key={c}
              onMouseDown={()=>handleSelect(c)}
              style={{padding:"9px 13px",cursor:"pointer",fontSize:14,display:"flex",justifyContent:"space-between",alignItems:"center",borderBottom:"0.5px solid rgba(0,0,0,0.06)"}}
              onMouseEnter={e=>e.currentTarget.style.background="#eef4ff"}
              onMouseLeave={e=>e.currentTarget.style.background=""}>
              <span style={{fontWeight:500}}>{c}</span>
              <span style={{fontSize:12,color:"#2563eb",fontWeight:500,background:"#eff6ff",padding:"2px 8px",borderRadius:10}}>{CPM[c]}</span>
            </div>
          ))}
        </div>
      )}
...
function LoginScreen({onLogin}){
  ...
  return(
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#f1f5f9",fontFamily:"sans-serif"}}>
      <div style={{background:"#fff",borderRadius:18,border:"1px solid rgba(0,0,0,0.1)",padding:"2rem",width:300,textAlign:"center"}}>
        <div style={{fontSize:36,marginBottom:8}}>🏭</div>
        <div style={{fontSize:20,fontWeight:700,marginBottom:4}}>ניהול ספקים</div>
        <div style={{fontSize:13,color:"#888",marginBottom:24}}>הזן את שמך להמשך</div>
        <input value={name} onChange={e=>setName(e.target.value)} onKeyDown={e=>e.key==="Enter"&&name.trim()&&onLogin(name.trim())}
          placeholder="שם משתמש..."
          style={{width:"100%",boxSizing:"border-box",padding:"10px 14px",fontSize:15,borderRadius:10,border:"1.5px solid rgba(0,0,0,0.18)",outline:"none",marginBottom:14,textAlign:"right",direction:"rtl"}}/>
        <button onClick={()=>name.trim()&&onLogin(name.trim())} disabled={!name.trim()}
          style={{width:"100%",padding:"11px",borderRadius:10,border:"none",background:name.trim()?"#1d4ed8":"#93c5fd",cursor:name.trim()?"pointer":"default",fontSize:15,fontWeight:600,color:"#fff"}}>
          כניסה
        </button>
      </div>
    </div>
...
export default function App(){
  ...
  async function handleLogin(name){
    setCurrentUser(name);
    await db.setMeta("currentUser",name);
  }
  ...
  const handleCardFile=async file=>{
    ...
    try{
      ...
      const txt=await callClaude("Business card. Extract: name (company/supplier), contact (person name), email, city (English city name). Return ONLY JSON with keys: name,contact,email,city. No markdown.",b64,mt);
      try{
        ...
        setForm(f=>({...f,name:p.name||f.name,contact:p.contact||f.contact,email:p.email||f.email,city:matchedCity||f.city,province:autoProvince||f.province}));
      }catch(_){}
  ...
  const handleSave=async()=>{
    ...
    try{
      ...
      if(selSup){
        await db.updateSupplier({...form,id:selSup.id,updatedAt:now,updatedBy:currentUser||"user"});
        setSelSup(s=>({...s,...form,updatedAt:now,updatedBy:currentUser||"user"}));
      } else {
        const id=await db.addSupplier({...form,createdAt:now,createdBy:currentUser||"user",updatedAt:now,updatedBy:currentUser||"user"});
  ...
  const handleSaveProd=async()=>{
    ...
    const data={...prodForm,supplierId:sid,updatedAt:now,updatedBy:currentUser||"user"};
    if(editProd)await db.updateProduct({...data,id:editProd.id});
    else await db.addProduct({...data,createdAt:now,createdBy:currentUser||"user"});
  ...
  const downloadZip=async()=>{
    ...
    try{
      ...
      a.download=`suppliers_${folder}.zip`;
      a.click();
  ...
  const importImages=async files=>{
    ...
    for(const file of Array.from(files)){
      ...
      if(parts.length===2&&parts[1]==="card"){
        ...
        if(!isNaN(sid)){const all=await db.getAllSuppliers();const sup=all.find(s=>s.id===sid);if(sup){const b64=await fileToBase64(file);await db.updateSupplier({...sup,cardImage:b64,cardImageType:file.type,updatedAt:nowISO(),updatedBy:currentUser||"import"});count++;}}
      } else if(parts.length>=2){
        const sid=parseInt(parts[0]),pid=parseInt(parts[1]);
        if(!isNaN(sid)&&!isNaN(pid)){const allP=await db.getAllProducts();const prod=allP.find(p=>p.id===pid&&p.supplierId===sid);if(prod){const b64=await fileToBase64(file);await db.updateProduct({...prod,images:[...(prod.images||[]),{data:b64,type:file.type}],updatedAt:nowISO(),updatedBy:currentUser||"import"});count++;}}
      }
    }
    ...
    <div dir="rtl" style={{fontFamily:"var(--font-sans)",padding:"0.75rem",maxWidth:560,margin:"0 auto",background:"var(--color-background-tertiary)",minHeight:"100vh"}}>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12,paddingTop:4}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          <span style={{fontSize:12,color:"#666",background:"#f1f5f9",padding:"4px 10px",borderRadius:20}}>👤 {currentUser}</span>
