  ...
  return(<div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"flex-end"}}>
    {images.map((img,i)=>(
      <div key={i} style={{position:"relative",flexShrink:0}}>
		...
		<img
		  src={img.url||`data:${img.type};base64,${img.data}`}
		  style={{
			width:72,
			height:72,
			objectFit:"contain",
    ...
    for (let si = 0; si < allS.length; si++) {
	  ...
	  if (dataUrl) {
		try {
		  ...
		  // מיקום ממורכז בתוך המסגרת
		  const imgX = margin + (frameW - fitted.width) / 2;
		  const imgY = y + (frameH - fitted.height) / 2;
      ...
      if (prods.length > 0) {
		...
		for (let i = 0; i < prods.length; i += 2) {
		  const rowItems = prods.slice(i, i + 2);
		  ...
		  const estimateProductHeight = async (p) => {
			let h = 8; // header
			const productDesc = isHebrewText(p.description || "")
			  ? rtl(p.description || "ללא תיאור")
			  : (p.description || "ללא תיאור");

			const descLines = doc.splitTextToSize(productDesc, colWidth - 8);
			h += descLines.length * 4 + 1;

			if (p.rating) h += 5;

			const imgs = (p.images || []).slice(0, 2).filter(img => img.url);
			if (imgs.length) h += 28;
		  ...
		  const heights = await Promise.all(rowItems.map(estimateProductHeight));
		  const rowHeight = Math.max(...heights, 24);

		  addPageIfNeeded(rowHeight + 4);

		  for (let col = 0; col < rowItems.length; col++) {
			const p = rowItems[col];
			const xBase = col === 0 ? rightX : leftX;

			let itemY = y;

			// מסגרת כרטיס מוצר
			doc.setDrawColor(230, 230, 230);
			doc.roundedRect(xBase, itemY, colWidth, rowHeight, 1.5, 1.5, "S");

			// כותרת מוצר
			doc.setFontSize(10);
			doc.setTextColor(...provinceColor);
			doc.text(rtl(`מוצר #${p.id}`), xBase + colWidth - 3, itemY + 5, { align: "right" });
			...
			const descLines = doc.splitTextToSize(productDesc, colWidth - 8);
			doc.text(descLines, xBase + 4, itemY, { align: "left" });
			itemY += descLines.length * 4 + 1;

			// דירוג
			if (p.rating) {
			  doc.setTextColor(180, 120, 0);
			  doc.text(`Rating: ${"★".repeat(p.rating)}`, xBase + 4, itemY, { align: "left" });
			  itemY += 5;
			}

			// תמונות - עד 2 לכל מוצר בגריד
			...
			for (const img of imgs) {
			  ...
			  try {
				...
				doc.roundedRect(imgX, itemY, frameW, frameH, 1, 1, "FD");

				const imgDrawX = imgX + (frameW - fitted.width) / 2;
				const imgDrawY = itemY + (frameH - fitted.height) / 2;
			...
			if (p.updated_at) {
			  ...
			  doc.text(productStamp, xBase + 4, itemY, { align: "left" });
			  itemY += 1;
			}
		  }
