...
/* ── Supabase config ── */
const SB_URL = "https://wphhfimcbgmnrqfucvlg.supabase.co";
const SB_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6IndwaGhmaW1jYmdtbnJxZnVjdmxnIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzU0MTc0MzUsImV4cCI6MjA5MDk5MzQzNX0.24i3JzmK7z6JGWV_k19lIkw6_Z5gq5ufXEXRrpGzFtw";
...
export default function App(){
  ...
  const downloadPDF=async()=>{
    ...
    try{
      ...
      function checkPage(need=10){if(y+need>285){doc.addPage();y=M;}}
      function drawLine(){doc.setDrawColor(220);doc.line(M,y,W-M,y);y+=4;}

      for(let si=0;si<allS.length;si++){
        const s=allS[si];
        if(si>0){doc.addPage();y=M;}
        // Supplier header
        doc.setFillColor(29,78,216);doc.rect(M,y,CW,10,"F");
        doc.setTextColor(255);doc.setFontSize(13);doc.setFont(undefined,"bold");
        doc.text(`#${s.id}  ${s.name||""}`,M+3,y+7);
        y+=14;doc.setTextColor(30);doc.setFont(undefined,"normal");
        // Card image
        if(s.card_image_url){
          try{
            const r=await fetch(s.card_image_url);const blob=await r.blob();
            const dataUrl=await new Promise(res=>{const fr=new FileReader();fr.onload=()=>res(fr.result);fr.readAsDataURL(blob);});
            doc.addImage(dataUrl,"JPEG",M,y,50,33);y+=37;
          }catch(_){}
        }
        // Info table
        const info=[
          ...
          ["מחוז",s.province],["תחום",(s.fields||[]).join(", ")],
          ["דירוג","★".repeat(s.rating||0)],["תיאור",s.description]
        ].filter(([,v])=>v);
        doc.setFontSize(10);
        for(const [k,v] of info){
          checkPage(7);
          doc.setFont(undefined,"bold");doc.text(k+":",M,y);
          doc.setFont(undefined,"normal");
          const lines=doc.splitTextToSize(String(v||""),CW-28);
          doc.text(lines,M+28,y);
          y+=5*lines.length;
        }
        // timestamps
        checkPage(8);doc.setFontSize(8);doc.setTextColor(160);
        if(s.created_at)doc.text(`נוצר ע"י ${s.created_by||""} · ${new Date(s.created_at).toLocaleString("he-IL")}`,M,y);y+=4;
        if(s.updated_at)doc.text(`עודכן ע"י ${s.updated_by||""} · ${new Date(s.updated_at).toLocaleString("he-IL")}`,M,y);y+=6;
        doc.setTextColor(30);

        // Products
        const prods=allP.filter(p=>p.supplier_id===s.id);
        if(prods.length>0){
          checkPage(12);drawLine();
          doc.setFontSize(12);doc.setFont(undefined,"bold");doc.text(`מוצרים (${prods.length})`,M,y);y+=7;
          for(const p of prods){
            checkPage(50);
            doc.setFillColor(245,247,250);doc.rect(M,y,CW,6,"F");
            doc.setFontSize(10);doc.setFont(undefined,"bold");doc.setTextColor(30);
            doc.text(`מוצר #${p.id}  ${"★".repeat(p.rating||0)}`,M+2,y+4.5);y+=8;
            doc.setFont(undefined,"normal");
            if(p.description){const lines=doc.splitTextToSize(p.description,CW-4);doc.text(lines,M+2,y);y+=5*lines.length;}
            // product images (max 3 per row)
            const imgs=p.images||[];
            if(imgs.length>0){
              checkPage(40);
              let ix=M;
              for(let ii=0;ii<Math.min(imgs.length,6);ii++){
                const img=imgs[ii];
                if(img.url){
                  try{
                    ...
                    doc.addImage(durl,"JPEG",ix,y,38,28);ix+=40;if(ix>W-M-38){ix=M;y+=30;checkPage(30);}
                  }catch(_){}
                }
              }
              if(ix>M)y+=32;
            ...
            if(p.updated_at)doc.text(`עודכן ע"י ${p.updated_by||""} · ${new Date(p.updated_at).toLocaleString("he-IL")}`,M,y);
            y+=6;doc.setTextColor(30);doc.setFontSize(10);
          }
        }
      }
